...
local function get_patch_metaset_patch_set_index(patch_metaset, patch_set_name)
	if patch_metaset.patch_set_indexes[patch_set_name] == nil then
		...
		data.raw["noise-expression"][patch_metaset.count_expression_name].expression =
			tne(patch_metaset.next_patch_set_index)
	end
...
local patch_blobbiness_enabled = false
...
local function resource_autoplace_settings(params)
	...
	local regular_spot_quantity_expression = noise.random_between(random_spot_size_minimum, random_spot_size_maximum)
		* regular_spot_quantity_base_at(distance)
	local regular_spot_radius_expression =
		noise.min(32, regular_rq_factor * regular_spot_quantity_expression ^ onethird)

	if params.has_starting_area_placement ~= nil then
		regular_blob_amplitude_maximum_distance = regular_blob_amplitude_maximum_distance
			+ regular_patch_fade_in_distance
	end
	local function regular_blob_amplitude_at(dist)
		return regular_blob_amplitude_multiplier
			* noise.min(
	...
	local starting_resource_placement_area = math.pi
	...
	local starting_feasibility = noise.clamp((elevation - 1) / 10, 0, 1) * starting_modulation
	...
	local basement_value = noise.min(-6 * regular_blob_amplitude_maximum, -6 * starting_blob_amplitude)
	...
	local starting_spots = tne({
		...
		arguments = {
			...
			spot_favorability_expression = litexp(
				starting_feasibility * 2 - 1 * distance / starting_resource_placement_radius + noise.random(0.5)
			),
			basement_value = basement_value,
	...
	local richness_distance_multiplier =
		noise.max(1, post_semd_richness_distance_multiplier_at(size_effective_distance_at(distance)))
